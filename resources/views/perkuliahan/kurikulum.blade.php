...
@php
    ...
    $tableTitles = [
        'tahun-kurikulum' => 'Daftar Tahun Kurikulum',
        'mata-kuliah' => 'Daftar Mata Kuliah',
        'kurikulum-prodi' => 'Mata Kuliah Kurikulum Prodi',
        'skala-nilai' => 'Skala Nilai',
        'komposisi-nilai' => 'Komposisi Nilai',
        'aturan-evaluasi' => 'Aturan Evaluasi Mata Kuliah',
        'ekivalensi-mata-kuliah' => 'Ekivalensi Mata Kuliah',
        'kurikulum-konsentrasi' => 'Kurikulum Konsentrasi',
        'prasyarat-mata-kuliah' => 'Prasyarat Mata Kuliah',
        'grup-mk-wajib-pilihan' => 'Grup Mata Kuliah Wajib dan Pilihan',
    ];
...
<style>
    ...
    .action-row{display:flex;flex-wrap:wrap;gap:8px}.danger{background:#ef4444}.danger:hover{background:#dc2626}.empty{border:1px dashed #cbd5e1;border-radius:12px;background:#f8fafc;padding:20px;text-align:center;color:#64748b;font-weight:800}
    @media(max-width:950px){.curriculum-layout,.stats,.cards,.form-grid{grid-template-columns:1fr}.curriculum-side{border-right:0;border-bottom:1px solid #e5e7eb}}
</style>
        ...
        <main class="curriculum-main">
            ...
            <div class="pad">
                @if($tab === 'tahun-kurikulum')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Tahun</th><th>Nama</th><th>Program Studi</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($curricula as $row)
                        <tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'curricula', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                            ...
                            <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'curricula', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                        </tr>
                ...
                @elseif(in_array($tab, ['mata-kuliah','aturan-evaluasi','grup-mk-wajib-pilihan'], true))
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kode</th><th>Nama</th><th>SKS</th><th>Lulus</th><th>Jenis</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($courses as $row)
                        <tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'courses', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                            ...
                            <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'courses', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                        </tr>
                ...
                @elseif(in_array($tab, ['kurikulum-prodi','kurikulum-konsentrasi'], true))
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kurikulum</th><th>MK</th><th>Semester</th><th>Paket</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($curriculumCourses as $row)
                        <tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'curriculum-courses', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                            ...
                            <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'curriculum-courses', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                        </tr>
                ...
                @elseif($tab === 'skala-nilai')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Huruf</th><th>Min</th><th>Max</th><th>Bobot</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($gradingScales as $row)<tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'grading-scales', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'grading-scales', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'komposisi-nilai')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Komponen</th><th>Persentase</th><th>Kelas</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($gradeComponents as $row)<tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'grade-components', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'grade-components', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'ekivalensi-mata-kuliah')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>MK Lama</th><th>MK Baru</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($courseEquivalences as $row)<tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'equivalences', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'equivalences', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'prasyarat-mata-kuliah')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Mata Kuliah</th><th>Prasyarat</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($coursePrerequisites as $row)<tr><form method="post" action="{{ route('curriculum.update', ['resource' => 'prerequisites', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('curriculum.destroy', ['resource' => 'prerequisites', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
