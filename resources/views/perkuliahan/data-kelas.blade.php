...
@php
    ...
    $tableTitles = [
        'tahun-ajaran' => 'Daftar Periode Akademik',
        'kelas-kuliah' => 'Daftar Kelas Kuliah',
        'detail-kelas-kuliah' => 'Detail Kelas Kuliah',
        'dosen-pengajar' => 'Dosen Pengajar Kelas',
        'jadwal-perkuliahan' => 'Jadwal Perkuliahan',
        'peserta-kelas' => 'Peserta Kelas',
        'presensi-kelas' => 'Rekap Presensi Kelas',
        'nilai-perkuliahan' => 'Nilai Perkuliahan',
        'jadwal-presensi' => 'Jadwal dan Presensi',
        'pemutihan-nilai' => 'Pemutihan Nilai',
    ];
...
<style>
    ...
    .action-row{display:flex;flex-wrap:wrap;gap:8px}.danger{background:#ef4444}.danger:hover{background:#dc2626}.muted-small{color:#64748b;font-size:12px;font-weight:700}
    @media(max-width:950px){.class-layout,.stats,.form-grid{grid-template-columns:1fr}.class-side{border-right:0;border-bottom:1px solid #e5e7eb}.form-grid .wide{grid-column:auto}}
</style>
        ...
        <main class="class-main">
            ...
            <div class="pad">
                @if($tab === 'tahun-ajaran')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kode</th><th>Nama</th><th>Tahun</th><th>Mulai</th><th>Selesai</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($periods as $row)
                        <tr><form method="post" action="{{ route('classes.update', ['resource' => 'periods', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                            ...
                            <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('classes.destroy', ['resource' => 'periods', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                        </tr>
                ...
                @elseif(in_array($tab, ['kelas-kuliah','detail-kelas-kuliah'], true))
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kelas</th><th>Prodi</th><th>MK</th><th>Periode</th><th>Kapasitas</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($classes as $row)
                        <tr><form method="post" action="{{ route('classes.update', ['resource' => 'classes', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                            ...
                            <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('classes.destroy', ['resource' => 'classes', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                        </tr>
                ...
                @elseif($tab === 'dosen-pengajar')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kelas</th><th>Dosen</th><th>Utama</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($classLecturers as $row)<tr><form method="post" action="{{ route('classes.update', ['resource' => 'lecturers', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('classes.destroy', ['resource' => 'lecturers', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'jadwal-perkuliahan')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kelas</th><th>Hari</th><th>Waktu</th><th>Ruang</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($schedules as $row)<tr><form method="post" action="{{ route('classes.update', ['resource' => 'schedules', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('classes.destroy', ['resource' => 'schedules', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'peserta-kelas')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kelas</th><th>Mahasiswa</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($classStudents as $row)<tr><form method="post" action="{{ route('classes.update', ['resource' => 'students', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        <td><select name="classId">@foreach($classes as $class)<option value="{{ $class->id }}" @selected(($row->classId ?? null) === ($class->id ?? null))>{{ $classLabel($class) ?: ($class->id ?? '-') }}</option>@endforeach</select></td><td><select name="studentId">@foreach($plainStudents as $student)<option value="{{ $student->id }}" @selected(($row->studentId ?? null) === ($student->id ?? null))>{{ $student->nim ?? '-' }} - {{ $student->name ?? '-' }}</option>@endforeach</select></td>
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('classes.destroy', ['resource' => 'students', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif(in_array($tab, ['presensi-kelas','jadwal-presensi'], true))
                    ...
                    @forelse($attendances as $row)<tr><form method="post" action="{{ route('classes.update', ['resource' => 'attendances', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        <td>{{ $row->courseCode ?? '-' }} - {{ $row->courseName ?? '-' }}</td><td><select name="meetingId">@foreach($meetings as $meeting)<option value="{{ $meeting->id }}" @selected(($row->meetingId ?? null) === ($meeting->id ?? null))>P{{ $meeting->meetingNo ?? '-' }} - {{ substr((string) ($meeting->meetingDate ?? ''),0,10) }}</option>@endforeach</select></td><td><select name="classStudentId">@foreach($classStudents as $student)<option value="{{ $student->id }}" @selected(($row->classStudentId ?? null) === ($student->id ?? null))>{{ $student->nim ?? '-' }} - {{ $student->studentName ?? '-' }}</option>@endforeach</select></td><td><select name="status">@foreach(['PRESENT','PERMIT','SICK','ABSENT'] as $status)<option value="{{ $status }}" @selected(($row->status ?? '') === $status)>{{ $status }}</option>@endforeach</select></td>
                ...
                @elseif(in_array($tab, ['nilai-perkuliahan','pemutihan-nilai'], true))
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>MK</th><th>Mahasiswa</th><th>Nilai</th><th>Huruf</th><th>Kunci</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($grades as $row)<tr><form method="post" action="{{ route('classes.update', ['resource' => 'grades', 'id' => $row->id, 'tab' => $tab]) }}">@csrf @method('PATCH')
                        <td>{{ $row->courseCode ?? '-' }} - {{ $row->courseName ?? '-' }}</td><td><select name="classStudentId">@foreach($classStudents as $student)<option value="{{ $student->id }}" @selected(($row->classStudentId ?? null) === ($student->id ?? null))>{{ $student->nim ?? '-' }} - {{ $student->studentName ?? '-' }}</option>@endforeach</select></td><td><input type="number" name="score" min="0" max="100" value="{{ $row->score ?? 0 }}"></td><td><input name="letter" value="{{ $row->letter ?? 'A' }}"></td><td><input type="checkbox" name="isLocked" value="1" style="width:auto" @checked(!empty($row->isLocked))></td>
