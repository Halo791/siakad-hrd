...
@php
    ...
    $tableTitles = [
        'daftar-pegawai' => 'Daftar Pegawai dan Dosen',
        'detail-pegawai' => 'Detail Pegawai dan Dosen',
        'pembimbing' => 'Pembimbing Akademik',
        'tanda-tangan' => 'Tanda Tangan Dosen',
        'kalender-akademik' => 'Kalender Akademik',
        'monitoring-kalender-akademik' => 'Monitoring Setting Kalender Akademik',
        'monitoring-mahasiswa' => 'Monitoring Mahasiswa oleh Orang Tua/Wali',
        'profil-alumni' => 'Profil Alumni dan Yudisium',
    ];
...
<div class="panel">
    <div class="portal-grid">
        <aside class="portal-side">
            ...
            <p class="muted" style="margin:0">CRUD data portal sesuai fungsi tiap submenu.</p>
        ...
        <main class="portal-main">
            ...
            <div class="pad">
                @if(in_array($tab, ['daftar-pegawai','detail-pegawai','tanda-tangan'], true))
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>NIDN</th><th>Nama</th><th>User</th><th>Prodi</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($lecturers as $row)<tr><form method="post" action="{{ route('portal.records.update', ['resource' => 'lecturers', 'id' => $row->id]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('portal.records.destroy', ['resource' => 'lecturers', 'id' => $row->id]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form>@if($tab === 'tanda-tangan')@include('partials.record-attachments', ['resource' => 'lecturers', 'table' => 'Lecturer', 'id' => $row->id, 'storeRoute' => 'portal.attachments.store', 'destroyRoute' => 'portal.attachments.destroy', 'titleLabel' => 'Keterangan', 'urlLabel' => 'Link tanda tangan', 'buttonText' => 'Simpan Link'])@endif</td>
                ...
                @elseif($tab === 'pembimbing')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Mahasiswa</th><th>Pembimbing</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($advisors as $row)<tr><form method="post" action="{{ route('portal.records.update', ['resource' => 'advisors', 'id' => $row->id]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('portal.records.destroy', ['resource' => 'advisors', 'id' => $row->id]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'kalender-akademik')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Kode</th><th>Nama</th><th>Mulai</th><th>Selesai</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($periods as $row)<tr><form method="post" action="{{ route('portal.records.update', ['resource' => 'periods', 'id' => $row->id]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('portal.records.destroy', ['resource' => 'periods', 'id' => $row->id]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'monitoring-kalender-akademik')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Periode</th><th>Prodi</th><th>KRS</th><th>Presensi</th><th>Aksi</th></tr></thead><tbody>
                ...
                @elseif($tab === 'monitoring-mahasiswa')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Mahasiswa</th><th>Nama Wali</th><th>Relasi</th><th>Telepon</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($parentRows as $row)<tr><form method="post" action="{{ route('portal.records.update', ['resource' => 'parents', 'id' => $row->id]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('portal.records.destroy', ['resource' => 'parents', 'id' => $row->id]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
                ...
                @elseif($tab === 'profil-alumni')
                    ...
                    <div class="table-wrap" data-title="{{ $tableTitles[$tab] ?? $tabLabels[$tab] ?? 'Tabel Data' }}"><table><thead><tr><th>Mahasiswa</th><th>Periode</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
                    @forelse($alumniRows as $row)<tr><form method="post" action="{{ route('portal.records.update', ['resource' => 'alumni', 'id' => $row->id]) }}">@csrf @method('PATCH')
                        ...
                        <td class="action-row"><button class="btn" type="submit">Update</button></form><form method="post" action="{{ route('portal.records.destroy', ['resource' => 'alumni', 'id' => $row->id]) }}">@csrf @method('DELETE')<button class="btn danger" type="submit">Hapus</button></form></td>
